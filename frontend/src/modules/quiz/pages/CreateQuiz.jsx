  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] p-2 md:p-10 flex items-center justify-center font-sans">
      ...
      <div className="w-full max-w-6xl bg-white rounded-[1.5rem] md:rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-slate-200 overflow-hidden flex flex-col lg:flex-row">
        ...
        <div className="flex-1 p-6 md:p-12 border-b lg:border-b-0 lg:border-r border-slate-100">
          <header className="mb-8 md:mb-10">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="font-bold text-[#1b8599] tracking-widest text-xs uppercase">
                New Assessment
              </span>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8">
              ...
              <div className="space-y-2">
                ...
                {quiz.mode === "EXAM" ? (
                  ...
                  <div
                    ...
                    className={`flex items-center justify-between px-5 py-3 md:py-4 rounded-2xl border-2 cursor-pointer transition-all duration-300 ${
                      quiz.allowAllAuthenticated
                  ...
                  >
                    ...
                    >
                      <div
                        className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${quiz.allowAllAuthenticated ? "right-1" : "left-1"}`}
                      />
                    </div>
                  </div>
                ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className="w-full bg-white px-4 py-3 rounded-xl border border-slate-200 text-sm font-bold outline-none focus:ring-2 ring-[#1b8599]/20"
                  />
                </div>
                <div className="space-y-1">
                  <span className="text-[10px] font-bold text-slate-400 ml-1">
            ...
            <div className="pt-4 flex flex-col md:flex-row gap-3">
              <button
                type="submit"
                className="flex-1 order-1 md:order-1 bg-slate-900 text-white py-4 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-slate-800 transition-all shadow-lg active:scale-95"
              >
                Launch Quiz
              </button>
              <button
                ...
                className="flex-1 order-2 md:order-2 py-4 bg-slate-100 text-slate-500 rounded-2xl font-bold text-sm hover:bg-slate-200 transition-all"
              >
                Cancel
              </button>
            </div>
        ...
        <div
          className={`w-full lg:w-[400px] bg-slate-50 p-6 md:p-8 flex flex-col transition-all duration-500 ${quiz.mode === "EXAM" && !quiz.allowAllAuthenticated ? "block" : "hidden"}`}
        >
          <div className="mb-6">
            <div className="flex items-center justify-between mb-1">
              <h3 className="font-black text-slate-900 uppercase text-sm tracking-tighter">
                Access Registry
              </h3>
              <span className="bg-[#ff9d5c] text-white text-[10px] px-2 py-0.5 rounded-full font-bold animate-pulse">
                ANTI-CHEAT
              </span>
            </div>
            <p className="text-xs text-slate-500 font-medium leading-relaxed">
              Limit entry to specific participants.
            </p>
          </div>
          ...
          <div className="space-y-3 mb-6">
            ...
            <label className="flex items-center justify-center gap-2 w-full py-3 bg-white border-2 border-dashed border-slate-300 rounded-2xl cursor-pointer hover:border-[#1b8599] hover:bg-[#1b8599]/5 transition-all group">
              ...
              <span className="text-xs font-bold text-slate-500 group-hover:text-[#1b8599]">
            ...
            <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] border border-slate-200 border-dashed overflow-hidden flex flex-col max-h-[400px] lg:max-h-[540px] shadow-inner">
              <div className="p-4 border-b border-slate-100 bg-white/80 flex justify-between items-center sticky top-0 z-10">
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] font-black text-[#1b8599]">
                    {quiz.allowedEmails.length} Users
                  </span>
                </div>
                {quiz.allowedEmails.length > 0 && (
                  <button
                    type="button"
                    onClick={() =>
                      setQuiz((prev) => ({ ...prev, allowedEmails: [] }))
                    }
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar bg-slate-50/30">
                {quiz.allowedEmails.length === 0 ? (
                  ...
                  quiz.allowedEmails.map((email, index) => (
                    <div
                      key={index}
                      className="flex items-center justify-between bg-white p-3 rounded-xl border border-slate-100 shadow-sm transition-all"
                    >
